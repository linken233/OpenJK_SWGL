...
float CG_ForceSpeedFOV( void )
{
	...
	if ( !cg.renderingThirdPerson && ((!cg.zoomMode && (cg_trueguns.integer || CG_ChangeViewDualWielding())) || cg.snap->ps.weapon == WP_SABER
...
static qboolean	CG_CalcFov( void ) {
	...
	else if ( (!cg.zoomMode || cg.zoomMode == 3) && (cg.snap->ps.forcePowersActive&(1<<FP_SPEED)) && player->client->ps.forcePowerDuration[FP_SPEED] )//cg.renderingThirdPerson &&
	{
		...
		else if ( !cg.renderingThirdPerson && ((!cg.zoomMode && (cg_trueguns.integer || CG_ChangeViewDualWielding())) || cg.snap->ps.weapon == WP_SABER
											   || cg.snap->ps.weapon == WP_MELEE) && cg_truefov.value )
		{
