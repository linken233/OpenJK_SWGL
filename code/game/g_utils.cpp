...
{//returns an entity that we can remove to prevent the game from overloading 
 ...
 //and then start searching for things that aren't mission critical to remove.
	...
	{
		if (!Q_stricmp(e->classname, "item_shield")
			|| !Q_stricmp(e->classname, "item_seeker")
			|| !Q_stricmp(e->classname, "misc_model_gun_rack")
			|| !Q_stricmp(e->classname, "misc_model_ammo_rack")
			|| !Q_stricmp(e->classname, "ammo_blaster")
			|| !Q_stricmp(e->classname, "ammo_powercell")
			|| !Q_stricmp(e->classname, "item_binoculars")
			|| !Q_stricmp(e->classname, "item_shield_lrg_instant")
			|| !Q_stricmp(e->classname, "item_medpak_instant"))
		{
			return e;
		}
	}
